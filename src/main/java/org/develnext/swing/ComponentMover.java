...
public class ComponentMover extends MouseAdapter
{
	...
	{
		for (Component component : components) {
            if (component instanceof DesignContainer) {
                for(Component e : ((DesignContainer) component).getComponents()) {
	...
	private void setupForDragging(MouseEvent e)
	{
        ...
        if (destination.getParent() instanceof DesignContainer)
            location = destination.getParent().getLocation();
        else
	...
	public void mouseDragged(MouseEvent e)
	{
		...
		while (Math.abs(locationX) < edgeInsets.left)
			locationX += snapSize.width;

		while (Math.abs(locationY) < edgeInsets.top)
			locationY += snapSize.height;

		Dimension d = new Dimension(Integer.MAX_VALUE, Integer.MAX_VALUE); // getBoundingSize( destination );
